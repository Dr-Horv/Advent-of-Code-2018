...
func AStar(start Coordinate, goal Coordinate, world map[Coordinate]*Room) (Path, error) {
...
func getNeighbours(coordinate Coordinate, world map[Coordinate]*Room) []Coordinate {
	neighbours := make([]Coordinate, 0)
	room := world[coordinate]
	if room.Up == Open {
		neighbours = append(neighbours, coordinate.Up())
	}

	if room.Down == Open {
		neighbours = append(neighbours, coordinate.Down())
	}

	if room.Left == Open {
		neighbours = append(neighbours, coordinate.Left())
	}

	if room.Right == Open {
		neighbours = append(neighbours, coordinate.Right())
	}
